...
const mockTraceList: Trace[] = Object.values(MOCK_TRACES);
...
export default function DashboardPage() {
  ...
  const filteredTraces = mockTraceList.filter((trace) => {
    if (filter === "all") return true;
    if (filter === "failed") return trace.status === "failed";
    return trace.status === "success";
  });

  const failedCount = mockTraceList.filter((t) => t.status === "failed").length;
  const successCount = mockTraceList.filter((t) => t.status === "success").length;
      ...
      <div className="max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <div className="text-2xl font-bold">{mockTraceList.length}</div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-4">
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-800"><h2 className="font-semibold">Recent Traces</h2></div>
